...
def main():
  ...
  vst3_bundle = build_dir + "\\Photo.vst3"
  ...
  if not zip:
    installer = "\\build-win\\installer\\Photo Installer.exe"

    if demo:
      installer = "\\build-win\\installer\\Photo Demo Installer.exe"

    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\build-win\\manual\\Photo manual.pdf"
    ]
    ...
    vst3_bundle = projectpath + "\\build-win\\Photo.vst3"
    if os.path.exists(vst3_bundle):
      add_folder_to_zip(zf, vst3_bundle, "Photo.vst3")

    # Add standalone executables
    files = [
      projectpath + "\\build-win\\Photo_x64.exe",
      projectpath + "\\build-win\\Photo_ARM64EC.exe",
    ]
    ...
    clap_files = [
      # Postbuild locations
      (projectpath + "\\build-win\\Photo_x64.clap", "Photo_x64.clap"),
      (projectpath + "\\build-win\\Photo_ARM64EC.clap", "Photo_ARM64EC.clap"),
      # Build output locations (fallback)
      (projectpath + "\\build-win\\clap\\x64\\Release\\Photo.clap", "Photo_x64.clap"),
      (projectpath + "\\build-win\\clap\\ARM64EC\\Release\\Photo.clap", "Photo_ARM64EC.clap"),
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\Photo-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Photo-vst3_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\Photo-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Photo-app_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\Photo-clap_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Photo-clap_ARM64EC.pdb"
  ]
